...
pub unsafe trait TypeErasedSystem<Passed, Return> {
    ...
    /// TODO: Doc comment
    unsafe fn acquire_and_run<'l>(&'l mut self, passed : Passed, world : &'l World) -> Pin<Box<dyn Future<Output = Return> + 'l>>;
...
unsafe impl<S : System<(), Passed = ()>, C : System<bool, Passed = ()>> TypeErasedSystem<(), ()> for ScheduledSystemConfig<S, C> {
    unsafe fn acquire_and_run<'l>(&'l mut self, _passed : (), world : &'l World) -> Pin<Box<dyn Future<Output = ()> + 'l>> {
        Box::pin(async {
            if let Some(run_if) = &mut self.run_if {
                // SAFETY: TODO
                if (! unsafe{ run_if.run.acquire_and_run((), world) }.await) {
                    return;
                }
            }
            // SAFETY: TODO
...
unsafe impl<C : System<bool, Passed = ()>> TypeErasedSystem<(), bool> for ConditionSystemConfig<C> {
    unsafe fn acquire_and_run<'l>(&'l mut self, _passed : (), world : &'l World) -> Pin<Box<dyn Future<Output = bool> + 'l>> {
        // SAFETY: TODO
